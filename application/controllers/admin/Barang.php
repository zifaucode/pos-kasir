...
class Barang extends CI_Controller
{
	...
	function tambah_barang()
	{
		if ($this->session->userdata('akses') == '1') {
			...
			$image_resource = Zend_Barcode::factory('code128', 'image', array('text' => $kobar), array())->draw();
			$image_name     = $kobar . '.jpg';
			$image_dir      = './assets/barcode/'; // penyimpanan file barcode
			imagejpeg($image_resource, $image_dir . $image_name);

			$this->m_barang->simpan_barang($kobar, $nabar, $kat, $satuan, $harpok, $harjul, $harjul_grosir, $stok, $min_stok, $image_name);
